...
def main() -> None:
    ...
    must(t, "CONTEXT_LENS_2D: tl.constexpr", "paged MQA 2D context-lens support")
    must(t, "q_offset = context_len - 1", "paged MQA exact per-token causal bound")
    must(t, "CONTEXT_LENS_2D=context_lens_c.ndim == 2", "paged MQA 2D launch specialization")
    ...
    must(t, "Preserve exact per-token KPool lengths for native MTP1.", "MTP1 exact KPool seq_lens")
    must_not(t, "seq_lens[:, -1].contiguous()", "lossy MTP seq_lens collapse")
