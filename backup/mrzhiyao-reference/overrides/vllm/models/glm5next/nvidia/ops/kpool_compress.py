...
def _kpool_softmax_rotate_write_cache_kernel(
    ...
    quantized_u8 = _encode_e4m3fn_u8(quantized)
    ...
    if WRITE_CACHE:
        ...
        tl.store(buf_fp8_ptr + out_k_offsets, quantized_u8, mask=mask)
...
def _kpool_tail_seed_kernel(
    ...
    """
    i = tl.program_id(0)
    raw_t = tl.load(tslot_ptr + i).to(tl.int64)
    safe_t = tl.maximum(raw_t, 0)
    blk = safe_t // KPOOL
    valid = (raw_t >= 0) & (blk < n_tail_blocks)
    if not valid:
        return
    ...
    safe_ahead = tl.maximum(ahead, 0)
    ahead_blk = safe_ahead // KPOOL
    if ahead >= 0 and ahead_blk < n_tail_blocks and ahead_blk == blk:
        return
    offs = tl.arange(0, BLOCK_D)
    m = offs < HEAD_DIM
    base = (blk * 2 * KPOOL + safe_t % KPOOL) * HEAD_DIM
...
def _kpool_decode_update_batched_kernel(
    ...
    for t in tl.range(0, NEXT_N):
        ...
        cache_valid = (cache_loc >= 0) & (cache_loc < KV_NUM_SLOTS)
        ...
        raw_block = tl.maximum(tail_slot, 0).to(tl.int64) // POOL_SIZE
        tail_valid = (tail_slot >= 0) & (raw_block < TAIL_NUM_BLOCKS)
        block = tl.minimum(raw_block, TAIL_NUM_BLOCKS - 1)
        block_base = block * TAIL_BLOCK_ELEMS
        pos_valid = cache_valid & (pos >= 0) & tail_valid
        ...
        if pos_valid & (slot == POOL_SIZE - 1):
            ...
            quantized_u8 = _encode_e4m3fn_u8(quantized)
            tl.store(buf_fp8_ptr + out_k_offsets, quantized_u8, mask=dim_mask)
...
def kpool_decode_update_and_maybe_write_cache_batched(
    ...
    _kpool_decode_update_batched_kernel[(num_requests,)](
        ...
        positions,
        next_n,
        TAIL_NUM_BLOCKS=tail_kv_cache.shape[0],
        KV_NUM_SLOTS=kv_cache.shape[0] * page_size,
        PAGE_SIZE=page_size,
