    ...
    def forward(
        ...
        **kwargs,
        ...
        if not get_pp_group().is_last_rank:
            # Inter-layer mHC fuses one layer's hc_post with the next layer's
            # hc_pre. A PP boundary cannot carry those implementation details,
            # so materialize hc_post here and let the next stage run standalone
            # hc_pre. This is mathematically equivalent to the unfused path.
            if post is not None:
