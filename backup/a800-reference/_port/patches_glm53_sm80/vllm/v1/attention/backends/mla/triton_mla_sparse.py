...
class TritonMLASparseImpl(XPUMLASparseImpl):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        # The split-KV fast kernel is specialized for the 576 rope-sparse
        # layout; the 512 NoPE layout (GLM-5.3-Flash) uses the generic
        # upstream interface instead.
        self._fast576 = self.head_size == _DIM_QK
    ...
    def _forward_bf16_kv(
        ...
        if self._fast576:
            ...
            # Generic upstream Triton reference path; supports the NoPE
            # latent-only layout (block_dpe=0), which is exactly the
            # GLM-5.3-Flash shape. d_v must be 512 per its assertion.
...
class TritonMLASparseBackend(AttentionBackend):
    ...
    def get_supported_head_sizes(cls) -> list[int]:
        # 576: DeepSeek-V3.2 / GLM-5.x rope-sparse layout (512 latent + 64 rope).
        # 512: GLM-5.3-Flash NoPE sparse layout (512 latent, qk_rope_head_dim=0)
        #      served through the upstream generic Triton interface.
        return [512, _DIM_QK]
