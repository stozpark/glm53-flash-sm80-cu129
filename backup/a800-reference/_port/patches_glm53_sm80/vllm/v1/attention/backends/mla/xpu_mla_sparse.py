...
class XPUMLASparseMetadata(AttentionMetadata):
    ...
    # Fields consumed by vLLM's unified MLA forward / kv-cache-update and by
    # the DSA indexer path. Defaults mirror FlashMLASparseMetadata; build()
    # fills the counting fields from the common metadata.
...
class XPUMLASparseMetadataBuilder(AttentionMetadataBuilder[XPUMLASparseMetadata]):
    ...
    def build(
        ...
        # Counts consumed by the unified MLA forward path and DSA indexer;
        # same split semantics as SparseMLACommonMetadataBuilder (no PCP).
...
class XPUMLASparseImpl(MLAAttentionImpl[XPUMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        topk_indices_global = triton_convert_req_index_to_global_index(
            ...
            # SM80 port: read the width dynamically from the buffer — the
            # kpool indexer widens it to topk + (kpool-1) rounded up to a
            # 128 multiple (e.g. 2048+3 -> 2176), while metadata.topk_tokens
            # keeps the config value (2048). Upstream model code documents
            # that the backend must use topk_indices.shape[1] here.
