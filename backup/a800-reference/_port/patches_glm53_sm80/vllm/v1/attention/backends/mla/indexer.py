...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        if num_decodes > 0:
            ...
            # SM80 port: `has_deep_gemm()` is a pure import check — the
            # vendored copy imports fine on Ampere but its CUDA kernels
            # reject SM80 at runtime ("Unsupported architecture"). Use the
            # arch-aware `is_deep_gemm_supported()` so pre-SM90 devices take
            # the Triton paged-MQA-logits path instead.
