...
def _sm80_triton_logits_enabled() -> bool:
    """SM80 port: route MQA logits through the PR-#47629 Triton kernels when
    DeepGEMM is unavailable for this architecture (e.g. Ampere, where the
    vendored deep_gemm imports fine but its CUDA kernels reject SM80)."""
...
def sparse_attn_indexer(
    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            if chunk.local_total_seq_lens == 0:
                ...
                elif _sm80_triton_logits_enabled():
                    # SM80 port: drop-in Triton replacement for DeepGEMM's
                    # non-paged fp8 MQA logits (LUT-decoded, no fp8 tl.dot).
    ...
    if has_decode:
        ...
        if current_platform.is_xpu():
            ...
            # SM80 port: PR-#47629 Triton paged MQA logits. It computes its
            # own scheduling, so deep_gemm's scheduler metadata is not needed;
            # context per request is shared across its next_n verify rows.
            context_lens_sm80 = (
...
class SparseAttnIndexer(CustomOp):
    ...
    def __init__(
        ...
        # SM80 port: PR-#47629 Triton logits kernels cover Ampere, so an
        # importable-but-unsupported deep_gemm no longer hard-fails here.
        if (
            current_platform.is_cuda()
            and not has_deep_gemm()
            and not _sm80_triton_logits_enabled()
        ):
            raise RuntimeError(
