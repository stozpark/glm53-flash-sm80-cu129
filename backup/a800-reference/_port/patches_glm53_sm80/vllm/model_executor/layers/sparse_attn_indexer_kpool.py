...
def _sm80_triton_logits_enabled() -> bool:
    ...
    DeepGEMM cannot run on this architecture — the vendored copy imports fine
    on Ampere but its CUDA kernels reject SM80 at runtime."""
...
def sparse_attn_indexer_kpool(
    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks if not short_prefill else ():
            ...
            if _sm80_triton_logits_enabled():
                # SM80 port: drop-in Triton replacement for DeepGEMM's
                # non-paged fp8 MQA logits (LUT decode, no fp8 tl.dot).
    ...
    if has_decode:
        ...
        if _sm80_triton_logits_enabled():
            # SM80 port: PR-#47629 Triton paged MQA logits; computes its own
            # scheduling (deep_gemm scheduler metadata not needed). The
            # per-request context is shared across its next_n verify rows.
            context_lens_sm80 = (
...
class SparseAttnIndexerKpool(CustomOp):
    ...
    def __init__(
        ...
        # SM80 port: Triton logits kernels cover Ampere, so an importable
        # but arch-unsupported deep_gemm no longer hard-fails here.
        if (
            current_platform.is_cuda()
