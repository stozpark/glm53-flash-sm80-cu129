...
def _fwht_quant_kernel(
    q_ptr,
    qout_ptr,  # SM80 port: BF16 sidecar; torch casts to float8_e4m3fn afterwards
    sout_ptr,
    n_rows,
    ...
    # SM80 port: store the pre-quant values as BF16; the Python wrapper casts
    # them with torch (software e4m3 encode, works on any arch). Storing
    # float8_e4m3fn directly needs Triton fp8e4nv support = SM89+ only.
    tl.store(qout_ptr + rows[:, None] * 128 + offs[None, :], y.to(tl.bfloat16), mask=rmask[:, None])
...
def fwht128_quant_fp8(q: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    SM80 port note: the kernel stores rotated+scaled values into a BF16
    sidecar; the final e4m3 bytes are produced by ``Tensor.to(float8_e4m3fn)``
    (PyTorch's software conversion runs on every arch), which keeps byte-level
    output identical to an in-kernel fp8 store on SM89+/SM90.
    ...
    n_rows = q.shape[0]
    q_sidecar_bf16 = torch.empty((n_rows, 128), dtype=torch.bfloat16, device=q.device)
    q_scale = torch.empty((n_rows, 1), dtype=torch.float32, device=q.device)
    if n_rows == 0:
        return torch.empty(
            (n_rows, 128), dtype=torch.float8_e4m3fn, device=q.device
        ), q_scale
    BLOCK_R = 32
    grid = (triton.cdiv(n_rows, BLOCK_R),)
    _fwht_quant_kernel[grid](
        q, q_sidecar_bf16, q_scale, n_rows, BLOCK_R=BLOCK_R, num_warps=2
    )
    q_fp8 = q_sidecar_bf16.to(torch.float8_e4m3fn)
...
def _kpool_softmax_rotate_write_cache_kernel(
    stage_k_ptr,  # SM80 port: BF16 [n_pools, HEAD_DIM] staging row (was buf_fp8)
    buf_fp32_ptr,
    slot_k_ptr,
    slot_score_ptr,
    ape_ptr,
    ...
    if WRITE_CACHE | RETURN_COMPRESSED:
        # SM80 port: one BF16 staging row per pool holds the quantized values;
        # the wrapper either scatters the e4m3 bytes into the paged cache or
        # returns them as a fresh fp8 tensor. Scale layout is unchanged.
        tl.store(
            stage_k_ptr + row * HEAD_DIM + offs,
        ...
        if WRITE_CACHE:
            ...
            out_s_offset = (
                loc.to(tl.int64) // PAGE_SIZE * BUF_NUMEL_PER_PAGE // 4
...
def kpool_compress_and_write_cache(
    ...
    n_pools = slot_k.shape[0]
    # SM80 port: the kernel stages quantized rows in BF16; e4m3 bytes are
    # produced here with PyTorch's arch-independent software conversion.
    stage_k = torch.empty((n_pools, head_dim), dtype=torch.bfloat16, device=slot_k.device)
    if return_compressed:
        compressed_scale = torch.empty(
            (n_pools,), dtype=torch.float32, device=slot_k.device
        )
    else:
        compressed_scale = buf_fp32  # unused by the kernel on this path

    _kpool_softmax_rotate_write_cache_kernel[(n_pools,)](
        stage_k,
        buf_fp32,
        slot_k,
        slot_score,
    ...
    if write_cache:
        # Scatter staged rows into the uint8 pool cache as raw e4m3 bytes.
        # SM80 port note: fully async — masked-off rows are redirected to
        # cache slot 0 (avoids `nonzero()`'s host sync). Duplicate-index
        # writes are made benign by first computing slot 0's FINAL bytes:
        # if any active row targets slot 0, every redirected row writes that
        # row's staged bytes, so all duplicate writes are byte-identical.
        flat_u8 = kv_cache.view(-1)
        offs_d = torch.arange(head_dim, device=slot_k.device, dtype=torch.int64)
        loc64 = loc.to(torch.int64)
        loc_safe = torch.where(write_mask, loc64, torch.zeros_like(loc64))
        idx_rows = loc_safe[:, None] * head_dim + offs_d[None, :]
        staged_bytes = stage_k.to(torch.float8_e4m3fn).view(torch.uint8)
        if has_write_mask:
            # Slot 0's final bytes, computed without any data-dependent
            # indexing (a GPU-tensor index forces a D2H sync, which breaks
            # CUDA-graph capture). Exactly one row can target slot 0, so the
            # masked byte-sum yields that row's staged bytes; blend with the
            # existing bytes by has0 ∈ {0, 1}.
            cand0 = (write_mask & (loc64 == 0)).to(torch.int32)
            has0 = cand0.sum()
            sel0 = (staged_bytes.to(torch.int32) * cand0[:, None]).sum(dim=0)
            vals0 = (
                flat_u8[:head_dim].to(torch.int32) * (1 - has0) + sel0 * has0
            ).to(torch.uint8)
            vals_rows = torch.where(
                write_mask[:, None], staged_bytes, vals0[None, :]
            ).view(torch.uint8)
        else:
            vals_rows = staged_bytes
        flat_u8.index_copy_(0, idx_rows.reshape(-1), vals_rows.reshape(-1))

    if return_compressed:
        compressed_k = stage_k.to(torch.float8_e4m3fn)
...
def _kpool_decode_update_batched_kernel(
    stage_k_ptr,  # SM80 port: BF16 [R*COMP_CAPACITY, HEAD_DIM] staging
    stage_s_ptr,  # SM80 port: FP32 [R*COMP_CAPACITY] staged scales
    comp_cnt_ptr,  # SM80 port: INT32 [R] completion counters
    comp_loc_ptr,  # SM80 port: INT64 [R*COMP_CAPACITY] pool cache locs
    tail_kv_ptr,
    ...
    for t in tl.range(0, NEXT_N):
        ...
        if pos_valid & (slot == POOL_SIZE - 1):
            ...
            # SM80 port: stage the completed pool instead of writing fp8
            # bytes in-kernel (float8e4nv stores need SM89+ Triton).
            comp_id = req.to(tl.int64) * COMP_CAPACITY + tl.atomic_add(
                comp_cnt_ptr + req, 1
            )
            tl.store(
                stage_k_ptr + comp_id * HEAD_DIM + offs,
                quantized.to(tl.bfloat16),
                mask=dim_mask,
            )
            tl.store(stage_s_ptr + comp_id, scale)
            tl.store(comp_loc_ptr + comp_id, loc)
...
def kpool_decode_update_and_maybe_write_cache_batched(
    ...
    page_size = kv_cache.shape[1]
    buf = kv_cache
    # SM80 port: staging buffers for completed pools; the e4m3 bytes are
    # materialized with PyTorch's software cast and scattered afterwards.
    comp_capacity = (next_n + pool_size - 1) // pool_size
    dev = key.device
    stage_k = torch.empty(
        (num_requests * comp_capacity, head_dim), dtype=torch.bfloat16, device=dev
    )
    stage_s = torch.empty(
        (num_requests * comp_capacity,), dtype=torch.float32, device=dev
    )
    comp_cnt = torch.zeros((num_requests,), dtype=torch.int32, device=dev)
    comp_loc = torch.full(
        (num_requests * comp_capacity,), -1, dtype=torch.int64, device=dev
    )
    ...
    # Scatter completed pools: K bytes at token slot, fp32 scale right after
    # the page's K region — byte layout identical to the original in-kernel
    # writes (fp32-view offset * 4 == byte offset).
    # SM80 port note: fully async — no `.any()` / boolean indexing here, both
    # force a device->host sync which CUDA-graph capture forbids
    # (cudaErrorStreamCaptureUnsupported). Incomplete rows are redirected to
    # cache slot 0; duplicate-index writes are made benign by computing slot
    # 0's FINAL bytes up front (the staged bytes of the completed row whose
    # loc == 0, if any), so every writer of slot 0 writes identical bytes.
    done = comp_loc >= 0
    loc64 = comp_loc.to(torch.int64)
    sel_loc = torch.where(done, loc64, torch.zeros_like(loc64))
    flat_u8 = kv_cache.view(-1)
    offs_d = torch.arange(head_dim, device=dev, dtype=torch.int64)
    idx_k = (sel_loc[:, None] // page_size) * buf.stride(0) + (
        sel_loc % page_size
    )[:, None] * head_dim + offs_d[None, :]
    staged_bytes = stage_k.to(torch.float8_e4m3fn).view(torch.uint8)
    # Slot 0's final bytes via pure arithmetic — GPU-tensor indexing
    # (`x[t]` with device t) forces a D2H sync that breaks graph capture.
    cand0 = (done & (loc64 == 0)).to(torch.int32)
    has0 = cand0.sum()
    sel0_k = (staged_bytes.to(torch.int32) * cand0[:, None]).sum(dim=0)
    vals0_k = (
        flat_u8[:head_dim].to(torch.int32) * (1 - has0) + sel0_k * has0
    ).to(torch.uint8)
    vals_k = torch.where(done[:, None], staged_bytes, vals0_k[None, :]).view(torch.uint8)
    flat_u8.index_copy_(0, idx_k.reshape(-1), vals_k.reshape(-1))

    s_byte_base = (
        (sel_loc // page_size) * buf.stride(0)
        + page_size * head_dim
        + (sel_loc % page_size) * 4
    )
    idx_s = s_byte_base[:, None] + torch.arange(4, device=dev, dtype=torch.int64)[None, :]
    vals_s_all = stage_s.view(torch.uint8).view(-1, 4)
    s0 = page_size * head_dim
    sel0_s = (vals_s_all.to(torch.int32) * cand0[:, None]).sum(dim=0)
    vals0_s = (
        flat_u8[s0 : s0 + 4].to(torch.int32) * (1 - has0) + sel0_s * has0
    ).to(torch.uint8)
    vals_s = torch.where(done[:, None], vals_s_all, vals0_s[None, :]).view(torch.uint8)
    flat_u8.index_copy_(0, idx_s.reshape(-1), vals_s.reshape(-1))
